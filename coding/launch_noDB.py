...
# 输出：匹配到的编码TAG 
# =============================================================================
def selcode(frases, threshold,dbName):
    if frases == 'NA;':
        return ''
    ...
    for verCut in verCuts:
        ...
        if len(a) > 0:
            ...
            #rsl = listPossible(strSQL,dbName)
            #print(rsl)
            #for s in rsl:
                #mapping+= s[0]+';'
    
    if mapping == '':
        query = tokenization(frases)
        query_bow = zidian.doc2bow(query)
...
for sht in wbRawdata:
    ...
    df['cut']=df['frase'].apply(lambda i: tokenization(i))   #调整相似度阈值
...
print('runtime: ' , wrTime) #with test data 0:37:10.408305
