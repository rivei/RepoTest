...
# 输出：按。/，分裂的短句字符串，以;分隔
# =============================================================================
def getPhrases(sentence, stopwords):
    if sentence == 'NA':
        return 'NA'
    ...
    if sentence != "":
        ...
        for x in s.sentences:
            outstr+= tokenization(x, stopwords) + ';'
    else:
        outstr = "无"
    return outstr
...
# 输出：分词的list
# =============================================================================
def tokenization(sentence, stopwords):
    result = []
...
# 输出：匹配到的编码TAG 
# =============================================================================
def selcode(frasesCut, threshold,dbName,zidian,lsi,lsi_vec,dfdic):
    if frasesCut == 'NA':
        return ''
    if frasesCut == '无':
        return '无'
...
#                                   主程序
# =============================================================================
#rawdata = "./data/K9777rnd_test.xlsx" #<----- input
#output = "result1.xlsx" #<-------- output
#def RunCore()
def RunCore(inputpath, outputpath):
    dbNew = "final.db"
    ...
    wbRawdata = pd.read_excel(io = inputpath, sheet_name = None) #读全部数据 
    xlsx = pd.ExcelWriter(outputpath)
    tempfile = pd.ExcelWriter('temp.xlsx') #save intermediate results
    ...
    for sht in wbRawdata:
        ...
        df['frase'] = df.apply(lambda row: getPhrases(row['cl01'],stopwords), axis = 1)
        print(Qno + '...finding codes...')
        #df['cut']=df['frase'].apply(lambda i: tokenization2(i))   #调整相似度阈值
        df['CodeLabel']=df['frase'].apply(lambda i: selcode(i, 0.6, dbNew,
          zidian,lsi,lsi_vec,dfdic))   #调整相似度阈值
        df.to_excel(tempfile, sheet_name=Qno,index = False)
        rsldf = df[['respid','Verbatim','CodeLabel']].copy()
        rsldf.to_excel(xlsx, sheet_name=Qno,index = False)
    xlsx.save()
    tempfile.save()
