# -*- coding: utf-8 -*-
"""
Created on Thu May  9 18:13:21 2019
基于Auto Encoder的文本聚类
https://www.kesci.com/home/project/5bfb8bdc954d6e0010676433
"""
...
#  使用Scikit-Learn获取 Newsgroups数据集
def read_NewsGroup_data(similarity):    
    ...
    if len(args) > 0:
        op.error("该脚本不需要参数设置.")
        sys.exit(1)
    categories_3NG = ['comp.graphics', 'alt.atheism','rec.sport.baseball', 'talk.politics.guns','rec.autos']
...
# 调用Correlation_Similarity作为 similarity数据集
trainSet_correlation = read_NewsGroup_data(Correlation_Similarity())
#Downloading 20news dataset. This may take a few minutes.
#2018-11-28 10:39:58,182 INFO Downloading 20news dataset. This may take a few minutes.
#Downloading dataset from https://ndownloader.figshare.com/files/5975967 (14 MB)
#2018-11-28 10:39:58,184 INFO Downloading dataset from https://ndownloader.figshare.com/files/5975967 (14 MB)

# 调用Cosine_Similarity作为similarity数据集
...
n_hidden_1 = int(Nn/2)         #------------ Auto Encoder之hidden layer 1.
n_hidden_2 = int(n_hidden_1/2) #------------ Auto Encoder之hidden layer 2.
n_hidden_3 = int(n_hidden_2/2) #------------ Auto Encoder之hidden layer 3.
n_code = str(2)                #------------ 输出层的维度
...
beta1 = 0.9 #------------------------------ 衰减率1.  
beta2 = 0.999 #---------------------------- 衰减率2.
learning_rate = (beta1/n_input) #---------- 学习率.
stop_learning = 1.35 #--------------------- 学习止点.
n_batch = math.ceil(sqrt(sqrt(n_input))) #- 每一个step的数据量
n_backpro = math.ceil(n_input/n_batch) #--- 每个epoch的反向传播量
n_clusters = 5 #--------------------------- 聚类数量
n_diplay = 10 #---------------------------- 在K-Means聚类时每隔多少步展示.

data_cor, labels_cor = trainSet_correlation.whole_dataset() #--------对数据和标签进行分配
data_cos, labels_cos = trainSet_cosine.whole_dataset()      #------- 对数据和标签进行分配


results_cor=[] #--------------------------- NMI得分list
loss_cost_cor=[] #------------------------- 损失list
steps_cor=[] #----------------------------- steps的list
...
results_cos=[] #--------------------------- NMI得分list
loss_cost_cos=[] #------------------------- 损失list
steps_cos=[] #----------------------------- steps的list
